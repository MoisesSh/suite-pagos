...
> **Revisión de `expert_database_conciliacion` (auditoría previa a traducción a
> Django):** se resolvieron directamente, por ser decisiones de modelado (no
> de negocio) según `supervision-modelos-bd`, los `related_name` faltantes,
> el `on_delete`/tipo de relación de `TransaccionLedger.referencia_evento`,
> el `on_delete` de las 3 FK nullable de `Discrepancia`, el tipo de
> `Discrepancia.tipo`/`.severidad`, el campo `ReporteERP.payload`, y se
> agregó `db_index=True` en `ConsultaConciliacionProveedor.referencia_corta`
> y `EventoPagoRecibido.procesado_at`. El único punto de negocio/arquitectura
> pendiente — el origen de `Discrepancia.resuelto_por` — fue confirmado por
> `suit-conciliacion` con el usuario (ver 3.4 y punto 9 en la sección 5):
> FK real a un modelo `Usuario` local propio de Conciliación. **Sección 3
> cerrada, lista para traducirse a modelos Django tal cual.**
...
- `procesado_at` — nullable hasta que el consumer Celery lo procese,
  **`db_index=True`** (añadido: el consumer filtra constantemente por
  `procesado_at__isnull=True` para tomar trabajo pendiente — mismo patrón
  que `EventoOutbox.estado` en 2.3).
...
- FK `evento` → `EventoPagoRecibido`, `CASCADE`, `related_name='consultas_conciliacion'` (hija dependiente del ciclo de vida del evento: no tiene sentido sin él).
- FK `banco` → `Banco`, `PROTECT`, `related_name='consultas_conciliacion'`.
- `referencia_corta` — clave de correlación con el `referencia` que el
  ...
  matching, **no** `cedula_pagador` (ver siguiente punto). **`db_index=True`**:
  es el campo por el que el servicio de matching filtra/cruza en cada
  intento de conciliación (alta cardinalidad, filtro constante — regla de
  la skill de supervisión §5), y el plan original no lo marcaba pese a
  describirlo como "el campo primario de matching".
...
- `referencia_evento` → FK real a `EventoPagoRecibido`, **`PROTECT`**, `related_name='transacciones_ledger'`. **Resuelto** (era una alternativa abierta: "FK o UUID lógico si se prefiere desacoplar" — no aplica la regla de "sin FK cruzada" de la sección 1, que rige solo entre Orquestador↔Conciliación; `EventoPagoRecibido` vive en esta misma base, así que corresponde FK real). `PROTECT` en vez de `CASCADE`: `TransaccionLedger` es la fuente contable del ledger (tabla de solo-inserción, §3.5) — nunca debe poder desaparecer en cascada porque se borre el evento que la originó; si algún día se necesita depurar eventos viejos, el ledger debe sobrevivir esa depuración.
- `created_at`.

**`LineaLedger`**
- FK `transaccion` → `TransaccionLedger`, `CASCADE`, `related_name='lineas'`.
- FK `cuenta` → `CuentaContable`, `PROTECT`, `related_name='lineas_ledger'`.
...
**`MovimientoBancario`**
- FK `banco` → `Banco`, `PROTECT`, `related_name='movimientos'`.
...
**`Discrepancia`**
- FK nullable a `MovimientoBancario` (`related_name='discrepancias'`), a
  `ConsultaConciliacionProveedor` (`related_name='discrepancias'`) y a
  `EventoPagoRecibido` (`related_name='discrepancias'`) — una discrepancia
  puede originarse en cualquiera de los lados del matching (incluyendo un
  `resultado_interpretado` en `monto_no_coincide`/`error_credenciales`/
  `pendiente_revision`, que siempre debe generar una `Discrepancia`, nunca
  fallar en silencio). **`on_delete=models.SET_NULL`** en las tres —
  **resuelto** (no estaba especificado): consistente con que ya son
  `null=True` por diseño (una discrepancia no depende exclusivamente de una
  sola fuente, así que no aplica `CASCADE`), y porque `Discrepancia` es en
  sí misma un registro de auditoría/excepción que debe sobrevivir aunque el
  origen que la disparó se depure o archive más adelante — perder la fila
  de discrepancia sería peor que perder la referencia a su origen.
- `tipo` — **`TextChoices`** (conjunto cerrado que gobierna lógica de
  alertas/routing, no un catálogo administrable — no hay indicio en el plan
  de que necesite atributos propios ni gestión sin deploy). **Resuelto**
  (no estaba tipado): valores sugeridos a definir por dominio de negocio —
  `sin_movimiento_bancario` (evento sin match en `MovimientoBancario`/
  `ConsultaConciliacionProveedor`), `monto_no_coincide`, `duplicado`,
  `error_proveedor`, `pendiente_revision_manual` — mapeando 1:1 con los
  valores relevantes de `resultado_interpretado` (3.2b) más los casos de
  `MovimientoBancario.estado_conciliacion = discrepante` sin evento asociado.
- `severidad` — **`TextChoices`** (`baja`, `media`, `alta`, `critica`) —
  mismo razonamiento: conjunto cerrado que gobierna la política de alerta
  obligatoria mencionada más abajo (p. ej. "alertar solo si severidad ≥
  media"), no un catálogo.
- `estado_resolucion` — `TextChoices` (`abierta`, `en_revision`, `resuelta`,
  `descartada`) — valores sugeridos, cerrado por ser el que gobierna el
  flujo de resolución.
- `resuelto_por` → FK real a `Usuario` (`apps/users/domain/models.py`,
  modelo de staff/admin propio de Conciliación con autenticación JWT
  local), `on_delete=models.SET_NULL`, `null=True, blank=True`,
  `related_name='discrepancias_resueltas'` (auditoría de quién resolvió).
  **`RESUELTO`** — confirmado por `suit-conciliacion` con el usuario: no
  viola la regla de "sin FK cruzada" de la sección 1 porque `Usuario` vive
  en la misma base de Conciliación, no en otro servicio.
...
- `fecha` — `DateTimeField` (momento del envío/generación del reporte).
- `referencia_externa` — `CharField`, identificador que el ERP asigna o
  espera para correlacionar el envío (recibo/lote), texto libre porque es
  un identificador de un sistema ajeno, no un concepto de catálogo propio.
- `payload` — **`JSONField`**. **Resuelto** (el plan original decía
  "`payload`/resumen" como alternativa sin decidir): se mantiene el crudo
  estructurado de lo efectivamente exportado (mismo patrón de staging/
  auditoría que `payload_crudo` en 3.2b y `EventoOutbox.payload` en 2.3),
  no un campo de texto libre de "resumen" — si se necesita una vista
  legible se deriva del JSON en la capa de servicio/serializer, no se
  duplica como un segundo campo de texto que puede desincronizarse del
  payload real.
...
9. **`RESUELTO` — Origen de `Discrepancia.resuelto_por` (Conciliación).**
   Añadido en la revisión de `expert_database_conciliacion` de la sección 3,
   confirmado por `suit-conciliacion` con el usuario: Conciliación mantiene
   su propio modelo de staff/admin local (`apps/users/domain/models.py::Usuario`,
   autenticación JWT propia de este servicio). `Discrepancia.resuelto_por`
   es FK real a ese `Usuario` local — no viola "ninguna FK cruzada entre las
   DBs" porque vive en la misma base de Conciliación, no en otro servicio.
